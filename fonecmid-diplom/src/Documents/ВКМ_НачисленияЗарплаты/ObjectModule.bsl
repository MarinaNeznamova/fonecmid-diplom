...
Процедура РассчитатьНачисление()
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Результат = ?(Выборка.План=0,0,Выборка.Оклад*Выборка.Факт/Выборка.План)+Выборка.ПроцентЗаРаботу;
...
Процедура РассчитатьОтпуск()
	...
	|	РАЗНОСТЬДАТ(ВКМ_ОсновныеНачисленияДанныеГрафика.ПериодДействияНачало, ВКМ_ОсновныеНачисленияДанныеГрафика.ПериодДействияКонец, ДЕНЬ) * 8 КАК ЧасыОтпускаКалендарные,
